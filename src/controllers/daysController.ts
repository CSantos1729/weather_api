...
import {generateLondonDaysData} from "../services/daysService.js";
...
export const getDays = async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (city === "london") {
        console.log(generateLondonDaysData());
        finalDaysData = generateLondonDaysData();
      } else {
      ...
      res.status(200).json(finalDaysData);
    } catch (error) {
